...
public class Explorer {
    ...
    public byte[] getRawblockFromHash(String hash) {
        String hexData = request("rawblock/" + hash + "?format=hex");
        return hexStringToByte(hexData);
    }
    ...
    public Block getBlockFromHash(NetworkParameters params, String hash) {
        byte[] rawBlock = getRawblockFromHash(hash); // Récupère les données brutes du bloc
        return fromRawblockToBlock(params, rawBlock); // Convertit les données brutes en un objet Block
    }
